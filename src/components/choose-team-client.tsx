...
export function ChooseTeamClient({ userName }: { userName: string }) {
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <Input value={teamName} onChange={(e) => setTeamName(e.target.value)} placeholder="Ej: Los Halcones FC" className="bg-card/50" />
                    </div>
                    <div>
                      <Label>Sigla (3-4 letras) *</Label>
                      <Input value={shortName} onChange={(e) => setShortName(e.target.value.toUpperCase())} placeholder="Ej: HFC" maxLength={4} className="bg-card/50 uppercase" />
                    </div>
                    <div>
                      <Label>Categoría *</Label>
                      <Input value={category} onChange={(e) => setCategory(e.target.value)} placeholder="Ej: Senior Amateur" className="bg-card/50" />
                    </div>
                  </div>
                  <div className="flex gap-2 mt-5">
                    <Button variant="outline" onClick={() => setView('choose')} className="flex-1" disabled={loading}>Volver</Button>
                    <Button onClick={handleCreateTeam} disabled={loading || !teamName || !shortName || !category} className="flex-1 bg-gradient-primary">
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <Input value={inviteCode} onChange={(e) => setInviteCode(e.target.value)} placeholder="Pega aquí el código o link" className="bg-card/50 font-mono text-sm" />
                    </div>
                  </div>
                  <div className="flex gap-2 mt-5">
                    <Button variant="outline" onClick={() => setView('choose')} className="flex-1" disabled={loading}>Volver</Button>
                    <Button onClick={handleJoinTeam} disabled={loading || !inviteCode} className="flex-1 bg-gradient-to-r from-sky-500 to-sky-600">
